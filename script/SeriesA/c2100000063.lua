...
function s.initial_effect(c)
	...
	aux.CreateLycansquadAlterLinkProc(c,2100000056)
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	local thg=Duel.GetMatchingGroup(Card.IsAbleToHand,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,nil)
...
function spfilter2(c,e,tp)
	return c:IsRace(RACE_BEASTWARRIOR) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP)
end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
    ...
    if tc:IsRelateToEffect(e) and Duel.SpecialSummon(tc,0,tp,tp,false,false,POS_FACEUP)>0 then
        ...
        if tc:IsCode(2100000056) and #g>0 and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
