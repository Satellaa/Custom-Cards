...
function s.initial_effect(c)
	...
	aux.CreateLycansquadAlterLinkProc(c,2100000054)
...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	Duel.SetPossibleOperationInfo(0,CATEGORY_TOGRAVE,nil,1,tp,LOCATION_HAND|LOCATION_ONFIELD)
end
function s.spop(e,tp,eg,ep,ev,re,r,rp)
    local tc=Duel.GetFirstTarget()
    if tc:IsRelateToEffect(e) and Duel.SpecialSummon(tc,0,tp,tp,false,false,POS_FACEUP)>0 then
        local g=Duel.GetMatchingGroup(Card.IsAbleToGrave,tp,LOCATION_HAND|LOCATION_ONFIELD,LOCATION_ONFIELD,nil)
        if tc:IsCode(2100000054) and #g>0 and 
