...
function(filter,_type,lv,extrafil,extraop,matfilter,stage2,location,forcedselection,specificmatfilter,requirementfunc,sumpos,extratg)
	...
	return	function(e,tp,eg,ep,ev,re,r,rp,chk)
				if chk==0 then
					...
					local BaseGroup=Duel.GetMatchingGroup(Ritual.Filter,tp,location,0,e:GetHandler(),filter,_type,e,tp,mg,mg2,func,specificmatfilter,lv,requirementfunc,sumpos)
...
function(filter,_type,lv,extrafil,extraop,matfilter,stage2,location,forcedselection,customoperation,specificmatfilter,requirementfunc,sumpos)
	...
	return	function(e,tp,eg,ep,ev,re,r,rp)
				...
				local ExtraLoc=nil
				local FinalGroup=Group.CreateGroup()
				local BaseLocGroup=Duel.GetMatchingGroup(aux.NecroValleyFilter(Ritual.Filter),tp,location,0,e:GetHandler(),filter,_type,e,tp,mg,mg2,func,specificmatfilter,lv,requirementfunc,sumpos)
				local ExtraLocSingleEffGroup=Duel.GetMatchingGroup(aux.NecroValleyFilter(Ritual.ExtraLocFilter),tp,LOCATION_NOTHAND,0,e:GetHandler(),filter,_type,e,tp,mg,mg2,func,specificmatfilter,lv,requirementfunc,sumpos,true)
				...
				if ExtraLocEff and ExtraLocEff:CheckCountLimit(tp) then
					ExtraLoc=ExtraLocEff:GetValue()
					local ReqFunc=ExtraLocEff:GetTarget() or false
					local ExtraLocGroup=Duel.GetMatchingGroup(aux.NecroValleyFilter(Ritual.ExtraLocFilter),tp,ExtraLoc,0,e:GetHandler(),filter,_type,e,tp,mg,mg2,func,specificmatfilter,lv,requirementfunc,sumpos,false,ReqFunc)
					FinalGroup:Merge(ExtraLocGroup)
				end
